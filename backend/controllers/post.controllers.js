...
export const commentOnPost = async (req, res) => {
    try {
        const { id:postId } = req.params; // ✅ Corrected destructuring
        const { text } = req.body;
...
export const getLikedPosts=async(req,res) =>{
    ...
    try{
        ...
        const likedPosts = await Post.where('_id').in(user.likedPosts).populate({path:'user',select:'-password'}).populate({path:'comments.user',select:'-password'});
